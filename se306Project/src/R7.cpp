...
class RandomWalk {
		...
		laserSub = nh.subscribe<sensor_msgs::LaserScan>("robot_7/base_scan", 1000, &RandomWalk::commandCallback, this);
		StageOdo_sub = nh.subscribe<nav_msgs::Odometry>("robot_7/odom",1000, &RandomWalk::StageOdom_callback,this);
		//
	...
	void StageOdom_callback(nav_msgs::Odometry msg,std_msgs::String sheepdogMsg)
	{
		...
		ROS_INFO("Robot 7 -- Current x position is: %f", px);
		ROS_INFO("Robot 7 -- Current y position is: %f", py);
		prevpx = px;
		prevpy = py;		
	};
...
//
	...
	void spin() {
		...
		while (ros::ok()) { // Keep spinning loop until user presses Ctrl+C
			std_msgs::String msg;
			std::stringstream ss;
			ss << "robot_7 -- px:" << px << " py:" << py << " theta:" << theta << " isRotate:" << isRotate;
    			msg.data = ss.str();
...
int main(int argc, char **argv) {
	ros::init(argc, argv, "RobotNode7"); // Initiate new ROS node named "RobotNode7"
	ROS_INFO("This node is: RobotNode7");
	ros::NodeHandle n;
	prevpx = 0;
	prevpx= 0;
