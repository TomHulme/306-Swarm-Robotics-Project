...
class RandomWalk {
	...
	void StageOdom_callback(nav_msgs::Odometry msg,std_msgs::String sheepdogMsg)
	{
		...
		ROS_INFO("Robot 2 -- Current x position is: %f", px);
		ROS_INFO("Robot 2 -- Current y position is: %f", py);
		prevpx = px;
		prevpy = py;		
	};
...
//
	...
	void spin() {
		...
		while (ros::ok()) { // Keep spinning loop until user presses Ctrl+C
			std_msgs::String msg;
			std::stringstream ss;
			ss << "robot_2 -- px:" << px << " py:" << py << " theta:" << theta << " isRotate:" << isRotate;
    			msg.data = ss.str();
...
int main(int argc, char **argv) {
	ros::init(argc, argv, "RobotNode2"); // Initiate new ROS node named "RobotNode2"
	ROS_INFO("This node is: RobotNode2");
	ros::NodeHandle n;
	prevpx = 0;
	prevpx= 0;
