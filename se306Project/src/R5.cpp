...
class RandomWalk {
		...
		ros::NodeHandle n;
		commandPub = nh.advertise<geometry_msgs::Twist>("robot_5/cmd_vel",1000);
		sheepPosPub = nh.advertise<std_msgs::String>("sheep_position",1000);
		// Subscribe to the simulated robot's laser scan topic and tell ROS to call
		// this->commandCallback() whenever a new message is published on that topic
		laserSub = nh.subscribe<sensor_msgs::LaserScan>("robot_5/base_scan", 1000, &RandomWalk::commandCallback, this);
		StageOdo_sub = nh.subscribe<nav_msgs::Odometry>("robot_5/odom",1000, &RandomWalk::StageOdom_callback,this);
		//
	...
	void StageOdom_callback(nav_msgs::Odometry msg,std_msgs::String sheepdogMsg)
	{
		...
		ROS_INFO("Robot 5 -- Current x position is: %f", px);
		ROS_INFO("Robot 5 -- Current y position is: %f", py);
		prevpx = px;
		prevpy = py;		
	};
...
//
	...
	void spin() {
		...
		while (ros::ok()) { // Keep spinning loop until user presses Ctrl+C
			std_msgs::String msg;
			std::stringstream ss;
			ss << "robot_5 -- px:" << px << " py:" << py << " theta:" << theta << " isRotate:" << isRotate;
    			msg.data = ss.str();
