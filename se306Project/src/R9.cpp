...
class RandomWalk {
		...
		ros::NodeHandle n;
		commandPub = nh.advertise<geometry_msgs::Twist>("robot_9/cmd_vel",1000);
		// Subscribe to the simulated robot's laser scan topic and tell ROS to call
		// this->commandCallback() whenever a new message is published on that topic
		laserSub = nh.subscribe<sensor_msgs::LaserScan>("robot_9/base_scan", 1000, &RandomWalk::commandCallback, this);
		StageOdo_sub = nh.subscribe<nav_msgs::Odometry>("robot_9/odom",1000, &RandomWalk::StageOdom_callback,this);
...
int main(int argc, char **argv) {
	ros::init(argc, argv, "RobotNode9"); // Initiate new ROS node named "random_walk"
	ros::NodeHandle n;
	prevpx = 0;
	prevpx= 0;
