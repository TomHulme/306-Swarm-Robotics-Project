...
class RandomWalk {
		...
		ros::NodeHandle n;
		NodeName << "sheep_" << number;
		commandPub = nh.advertise<geometry_msgs::Twist>(NodeName.str() + "/cmd_vel",1000);
		sheepPosPub = nh.advertise<std_msgs::String>(NodeName.str() + "/sheep_position",1000);
		// Subscribe to the simulated robot's laser scan topic and tell ROS to call
		// this->commandCallback() whenever a new message is published on that topic
		laserSub = nh.subscribe<sensor_msgs::LaserScan>(NodeName.str() + "base_scan", 1000, &RandomWalk::commandCallback, this);
		StageOdo_sub = nh.subscribe<nav_msgs::Odometry>(NodeName.str() + "odom",1000, &RandomWalk::StageOdom_callback,this);
	...
	void spin() {
		...
		while (ros::ok()) { // Keep spinning loop until user presses Ctrl+C
			std_msgs::String msg;
			std::stringstream ss;
			ss << "sheep_" << number << " -- px:" << px << " py:" << py << " theta:" << theta << " isRotate:" << isRotate;
    			msg.data = ss.str();
...
int main(int argc, char **argv) {
	ros::param::get("~sheep/number", number);
	ros::param::set("~sheep/number", number+1);
	
	s << "SheepNode" << number;
	ros::init(argc, argv, s.str()); // Initiate new ROS node named "random_walk"
	ros::NodeHandle n;
	prevpx = 0;
	prevpx= 0;
