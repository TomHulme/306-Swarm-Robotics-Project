...
class Grass {
	...
	void spin() {
		...
		while (ros::ok()) { // Keep spinning loop until user presses Ctrl+C
			std_msgs::String msg;
			std::stringstream ss;
			ss << "Grass1 -- px:" << px << " py:" << py << " theta:" << theta << " isRotate:" << isRotate;
    			msg.data = ss.str();
...
int main(int argc, char **argv) {
	ros::init(argc, argv, "Grass1"); // Initiate new ROS node named "RobotNode<X>" where <X> is the number of the node
	ros::NodeHandle n;
	prevpx = 0;
	prevpx= 0;
