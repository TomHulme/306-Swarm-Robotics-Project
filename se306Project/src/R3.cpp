...
class RandomWalk {
		...
		ros::NodeHandle n;
		commandPub = nh.advertise<geometry_msgs::Twist>("robot_3/cmd_vel",1000);
		// Subscribe to the simulated robot's laser scan topic and tell ROS to call
		// this->commandCallback() whenever a new message is published on that topic
		laserSub = nh.subscribe<sensor_msgs::LaserScan>("robot_3/base_scan", 1000, &RandomWalk::commandCallback, this);
		StageOdo_sub = nh.subscribe<nav_msgs::Odometry>("robot_3/odom",1000, &RandomWalk::StageOdom_callback,this);
...
int main(int argc, char **argv) {
	ros::init(argc, argv, "RobotNode3"); // Initiate new ROS node named "random_walk"
	ros::NodeHandle n;
	prevpx = 0;
	prevpx= 0;
...
//to stage
ros::Publisher RobotNode_stage_pub = n.advertise<geometry_msgs::Twist>("robot_15/cmd_vel",1000); 

//subscribe to listen to messages coming from stage
ros::Subscriber StageOdo_sub = n.subscribe<nav_msgs::Odometry>("robot_15/odom",1000, StageOdom_callback);
ros::Subscriber StageLaser_sub = n.subscribe<sensor_msgs::LaserScan>("robot_15/base_scan",1000,StageLaser_callback);
